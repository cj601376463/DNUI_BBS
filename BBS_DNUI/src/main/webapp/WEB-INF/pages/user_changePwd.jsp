...
<div class="panel panel-danger">
    ...
    <div class="panel-body">
        <form class="form-inline" style="margin: 25px" action="${path}/user/changePwdInfo">
            <!--一个隐藏域，用于修改方便使用：-->
            <input name="user_id" value="<%=user.getUser_id()%>" type="hidden">

            <label><strong>原密码：</strong>
            <input name="initPwd" type="password" class="form-control" required><span id="msg"></span></label>
            <br/><br/>

            <label><strong>新密码：</strong>
            <input name="password" type="password" class="form-control" required minlength="6"></label>

            <label><strong>确认密码：</strong>
            <input name="repassword" type="password" class="form-control" minlength="6"></label>

            <label><input class="btn btn-danger" type="submit" id="submit" value="确认修改"/></label>
        </form>
    </div>
</div>
...
<script>
    $(function () {
        $("input[name='initPwd']").blur(function () {
            $.ajax({
                type: 'GET',
                url: '${path}/user/checkInitPwd?user_id=' + $("input[name='user_id']").val() + '&initPwd=' + $("input[name='initPwd']").val(),
